...
function Start-DevServer {
    ...
    if (Test-Path "limebird-site-new") {
        Set-Location "limebird-site-new"
        Write-Status "Changed to limebird-site-new directory"
    }
    ...
    # Use Start-Process with proper parameters for development server
    ...
    if ($process -and -not $process.HasExited) {
        Write-Status "Development server process started successfully" "SUCCESS"
    } else {
        Write-Status "Development server process may have failed to start" "WARNING"
    }
...
function Test-Server {
    ...
    $portInUse = netstat -ano | findstr ":$Port "
    if (-not $portInUse) {
        Write-Status "Port $Port is not in use - server may not be running" "ERROR"
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            Write-Status "Site is accessible at: $Url" "SUCCESS"
            return $true
        } else {
